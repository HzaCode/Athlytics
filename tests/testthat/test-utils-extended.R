...
test_that("sample data exists", {
  ...
  expect_true(!is.null(sample_ef))
  expect_true(!is.null(sample_exposure))
  expect_true(!is.null(sample_decoupling))
  expect_true(!is.null(sample_pbs))
})
...
test_that("package datasets are properly formatted", {
  ...
  expect_true("date" %in% names(sample_acwr))
  expect_true("acwr" %in% names(sample_acwr) || "acwr_smooth" %in% names(sample_acwr))
})
