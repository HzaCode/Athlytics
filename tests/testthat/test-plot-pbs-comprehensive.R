...
test_that("plot_pbs handles pre-calculated data", {
  ...
  p1 <- plot_pbs(pbs_df = sample_pbs)
  expect_s3_class(p1, "ggplot")

  # Test with add_trend_line = FALSE
  p2 <- plot_pbs(pbs_df = sample_pbs, add_trend_line = FALSE)
  expect_s3_class(p2, "ggplot")

  # Test with add_trend_line = TRUE
  p3 <- plot_pbs(pbs_df = sample_pbs, add_trend_line = TRUE)
...
test_that("plot_pbs handles different activity types", {
  ...
  p_multi <- plot_pbs(pbs_df = sample_pbs, activity_type = c("Run", "Ride"))
...
test_that("plot_pbs handles different distance configurations", {
  ...
  p1 <- plot_pbs(pbs_df = sample_pbs, distance_meters = 1000)
  expect_s3_class(p1, "ggplot")

  # Test with multiple distances
  p2 <- plot_pbs(pbs_df = sample_pbs, distance_meters = c(1000, 5000, 10000))
  expect_s3_class(p2, "ggplot")

  # Test with different distance combinations
  p3 <- plot_pbs(pbs_df = sample_pbs, distance_meters = c(500, 1500, 3000))
  expect_s3_class(p3, "ggplot")
})
...
test_that("plot_pbs handles max_activities parameter", {
  ...
  p1 <- plot_pbs(pbs_df = sample_pbs, max_activities = 50)
  expect_s3_class(p1, "ggplot")

  p2 <- plot_pbs(pbs_df = sample_pbs, max_activities = 200)
  expect_s3_class(p2, "ggplot")

  p3 <- plot_pbs(pbs_df = sample_pbs, max_activities = 1000)
  expect_s3_class(p3, "ggplot")
})
