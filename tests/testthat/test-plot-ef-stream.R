# Test for plot_ef.R to boost coverage
...
test_that("plot_ef handles different smoothing scenarios", {
  ...
  p1 <- plot_ef(sample_ef, smoothing_method = "loess", add_trend_line = TRUE)
  expect_s3_class(p1, "ggplot")

  p2 <- plot_ef(sample_ef, smoothing_method = "lm", add_trend_line = FALSE)
  expect_s3_class(p2, "ggplot")

  p3 <- plot_ef(sample_ef, smoothing_method = "gam", add_trend_line = TRUE)
...
test_that("plot_ef handles activity type filtering edge cases", {
  ...
  p2 <- plot_ef(sample_ef, activity_type = "Swim")
  expect_s3_class(p2, "ggplot")

  # Test with multiple activity types
  p3 <- plot_ef(sample_ef, activity_type = c("Run", "Ride"))
  expect_s3_class(p3, "ggplot")
})
...
test_that("plot_ef handles date filtering edge cases", {
  ...
  p3 <- plot_ef(sample_ef,
    start_date = Sys.Date() - 30
  )
  expect_s3_class(p3, "ggplot")

  # Test with only end_date
  p4 <- plot_ef(sample_ef,
    end_date = Sys.Date()
  )
...
test_that("plot_ef handles ef_metric edge cases", {
  ...
  p1 <- plot_ef(sample_ef, ef_metric = "pace_hr")
  expect_s3_class(p1, "ggplot")

  # Test with power_hr metric
  p2 <- plot_ef(sample_ef, ef_metric = "power_hr")
  expect_s3_class(p2, "ggplot")
})
