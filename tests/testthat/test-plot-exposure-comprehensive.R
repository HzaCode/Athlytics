...
test_that("plot_exposure handles pre-calculated data", {
  ...
  p2 <- plot_exposure(exposure_df = sample_exposure, risk_zones = FALSE)
  expect_s3_class(p2, "ggplot")

  # Test with risk_zones = TRUE
  p3 <- plot_exposure(exposure_df = sample_exposure, risk_zones = TRUE)
...
test_that("plot_exposure handles different activity types", {
  ...
  p_run <- plot_exposure(exposure_df = sample_exposure, activity_type = "Run")
  expect_s3_class(p_run, "ggplot")

  # Test with multiple activity types
  p_multi <- plot_exposure(exposure_df = sample_exposure, activity_type = c("Run", "Ride"))
